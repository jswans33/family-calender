...
test.describe('Event Time Display', () => {
  test('should display meal events with correct times instead of "All day"', async ({
    page,
  }) => {
    ...
    if (await morganEvent.isVisible()) {
      const eventContainer = morganEvent.locator(
        'xpath=ancestor::div[contains(@class, "cursor-pointer")]'
      );
      const timeText = await eventContainer
        .locator('div.text-xs')
        .nth(1)
        .textContent();
    ...
    if (await indianEvent.isVisible()) {
      const eventContainer = indianEvent.locator(
        'xpath=ancestor::div[contains(@class, "cursor-pointer")]'
      );
      const timeText = await eventContainer
        .locator('div.text-xs')
    ...
    if (await macEvent.isVisible()) {
      const eventContainer = macEvent.locator(
        'xpath=ancestor::div[contains(@class, "cursor-pointer")]'
      );
      const timeText = await eventContainer
        .locator('div.text-xs')
    ...
    const allDaySection = page
      .locator('.bg-gray-50')
      .filter({ hasText: 'All day' });
    if ((await allDaySection.count()) > 0) {
      // Make sure meal events are NOT in the all-day section
      const allDayEvents = await allDaySection
        .locator('text=/Morgan|Indian|Mac/')
        .count();
      expect(allDayEvents).toBe(0);
    }

    // Verify events appear in timed slots
    const timedEvents = page
      .locator('.absolute')
      .filter({ hasText: /Morgan|Indian|Mac/ });
  ...
  test('should correctly parse 24-hour time format from API', async ({
    page,
  }) => {
