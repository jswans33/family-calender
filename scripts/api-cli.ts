  ...
  .action(async (options) => {
    ...
    try {
      if (options.calendar) {
        ...
        const response = await fetch(`${API_BASE}/events?calendar=${options.calendar}`);
        const events = await response.json();
        console.log(`Found ${events.length} events in ${options.calendar}`);
        
        events.slice(0, 5).forEach((event: any) => {
          console.log(`  - ${event.title} (${event.date}) ${event.calendar_name ? `[${event.calendar_name}]` : ''}`);
        });
      } else {
        // Read all calendars
...
program
  ...
  .action(async (eventId) => {
    ...
    try {
      ...
      const encodedId = Buffer.from(eventId).toString('base64').replace(/[+/=]/g, match => {
