...
async function pullAllCalendarData() {
  ...
  try {
    ...
    calendars.forEach(calendar => {
      console.log(`📅 ${calendar.displayName.padEnd(8)} | ${calendar.count.toString().padStart(3)} events | ${calendar.path}`);
    });
    
    const totalFound = calendars.reduce((sum, cal) => sum + Math.max(0, cal.count), 0);
    ...
    const breakdownByCalendar = calendars.map(cal => {
      const calendarEvents = allEvents.filter(e => e.calendar_name === cal.name);
      return {
    ...
    breakdownByCalendar.forEach(cal => {
      console.log(`📅 ${cal.name.toUpperCase().padEnd(8)} | ${cal.count.toString().padStart(3)} events`);
      
      // Show first 3 events as samples
      cal.events.slice(0, 3).forEach((event, index) => {
        console.log(`   ${index + 1}. ${event.title.substring(0, 40)}${event.title.length > 40 ? '...' : ''}`);
    ...
    if (events2023.length > 0) {
      console.log(`   Sample: "${events2023[0].title}" (${events2023[0].calendar_name})`);
    }
    console.log('');
    ...
    console.log('   await repo.createEvent(newEvent, "/home/")  // Creates in Home calendar');
    console.log('   await repo.createEvent(newEvent, "/work/")  // Creates in Work calendar\\n');
    
    console.log('🗑️  DELETE from specific calendar:');
    console.log('   const event = allEvents.find(e => e.id === "target-id")');
    console.log('   await repo.deleteEvent(event.id, event.calendar_path, event.caldav_filename)\\n');
    ...
    console.log('   await repo.updateEvent(updatedEvent, event.calendar_path)\\n');
    
    // Step 7: Next steps
    console.log('7️⃣  NEXT STEPS FOR INTEGRATION:\\n');
    console.log('✅ 1. Update DatabaseCalendarService to use CalDAVMultiCalendarRepository');
    console.log('✅ 2. Add calendar_path and calendar_name columns to SQLite database');
    ...
    console.log('🎯 SUCCESS: All 4 calendars accessible with full CRUD operations!');
