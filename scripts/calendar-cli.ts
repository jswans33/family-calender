...
program
  ...
  .requiredOption(
    '-c, --calendar <name>',
    'calendar name (home, work, shared, meals)'
  )
  ...
  .action(async options => {
    ...
    const { title, date, calendar, time, duration, description, location } =
      options;
    ...
    try {
      ...
      if (options.localOnly) {
        ...
        try {
          await execAsync(
            `sqlite3 /home/james/projects/swanson-light/data/calendar.db "${sql}"`
          );
          console.log(
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `✅ SUCCESS: Event "${title}" created in ${calendar} calendar`
            );
    ...
    try {
      if (options.calendar) {
        ...
        const events = allEvents.filter(
          e => e.calendar_name === options.calendar
        );
...
program
  ...
  .action(async (eventId, options) => {
    ...
    try {
      ...
      const success = await repo.createEventInCalendar(
        updatedEvent,
        options.calendar
      );
      ...
      if (success) {
        console.log(
          `✅ SUCCESS: Event updated in ${options.calendar} calendar`
        );
...
program
  ...
  .action(async (eventId, options) => {
    ...
    try {
      ...
      const calendar = repo['calendars'].find(
        (cal: any) => cal.name === options.calendar
      );
      if (!calendar) {
      ...
      if (response.ok || response.status === 404) {
        console.log(
          `✅ SUCCESS: Event deleted from ${options.calendar} calendar`
        );
      } else {
...
program
  ...
  .action(async () => {
    ...
    for (const calendar of CALENDARS) {
      ...
      try {
        const calendarObj = repo['calendars'].find(
          (cal: any) => cal.name === calendar
        );
