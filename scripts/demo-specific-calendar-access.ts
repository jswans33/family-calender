...
async function demoSpecificCalendarAccess() {
  ...
  try {
    ...
    homeEvents.slice(0, 5).forEach((event, index) => {
      console.log(`   ${index + 1}. ${event.title} (${new Date(event.date).toLocaleDateString()})`);
    });
  ...
  try {
    const sharedXmlData = await repo.fetchCalendarData('/2D7581FA-3A83-42D8-B6F4-8BCD8186AA6E/');
  ...
  try {
    const mealsXmlData = await repo.fetchCalendarData('/1fa1e4097e27af6d41607163c20c088e70cf8e9db9d71b1a62611ec364123914/');
  ...
  console.log('await repo.createEvent(sampleEvent, "/2D7581FA-3A83-42D8-B6F4-8BCD8186AA6E/")');
  console.log('');
  console.log('// Create in MEALS calendar:');
  console.log('await repo.createEvent(sampleEvent, "/1fa1e4097e27af6d41607163c20c088e70cf8e9db9d71b1a62611ec364123914/")');
  ...
  try {
    ...
    const sharedXmlData = await repo.fetchCalendarData('/2D7581FA-3A83-42D8-B6F4-8BCD8186AA6E/');
    const sharedEventsWithFilenames = repo.parseCalendarEventsWithFilenames(sharedXmlData);
    
    sharedEventsWithFilenames.forEach((item, index) => {
      console.log(`   ${index + 1}. Event: "${item.event.title}"`);
      console.log(`      📄 Filename: ${item.filename}`);
      console.log(`      🗑️  Delete code: await repo.deleteEvent("${item.event.id}", "/2D7581FA-3A83-42D8-B6F4-8BCD8186AA6E/", "${item.filename}")`);
    });
  } catch (error) {
  ...
  console.log('   • MEALS: /1fa1e4097e27af6d41607163c20c088e70cf8e9db9d71b1a62611ec364123914/ (1 event)');
