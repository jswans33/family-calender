...
async function verifyCRUDOperations() {
  ...
  // Log function
  const log = (operation: string, calendar: string, eventId: string, result: boolean, details: string) => {
  ...
  const calendars = [
    ...
    { name: 'shared', path: '/2D7581FA-3A83-42D8-B6F4-8BCD8186AA6E/', event: testEvents.shared },
    { name: 'meals', path: '/1fa1e4097e27af6d41607163c20c088e70cf8e9db9d71b1a62611ec364123914/', event: testEvents.meals }
  ];
  ...
  try {
    ...
    for (const cal of calendars) {
      try {
        ...
        log('CREATE', cal.name, cal.event.id, createResult, `Event created in ${cal.name} calendar`);
      } catch (error) {
    ...
    for (const cal of calendars) {
      try {
        ...
        log('READ', cal.name, cal.event.id, !!found, found ? `Found: "${found.title}"` : 'Event not found');
      } catch (error) {
    ...
    for (const cal of calendars) {
      try {
        ...
        log('UPDATE', cal.name, cal.event.id, updateResult, `Title updated to include "UPDATED"`);
    ...
    for (const cal of calendars) {
      try {
        ...
        const eventToDelete = eventsWithFiles.find(item => item.event.id === cal.event.id);
        
        if (eventToDelete) {
          const deleteResult = await repo.deleteEvent(cal.event.id, cal.path, eventToDelete.filename);
          log('DELETE', cal.name, cal.event.id, deleteResult, `File: ${eventToDelete.filename}`);
        } else {
          log('DELETE', cal.name, cal.event.id, false, 'Event not found for deletion');
        }
      } catch (error) {
    ...
    for (const cal of calendars) {
      try {
        ...
        log('VERIFY', cal.name, cal.event.id, !stillExists, stillExists ? 'Event still exists!' : 'Event properly cleaned up');
      } catch (error) {
  ...
  calendarNames.forEach(calName => {
    const results = operations.map(op => {
      const result = testResults.find(r => r.operation === op && r.calendar === calName);
      return result ? (result.result === '✅ PASS' ? '✅' : '❌') : '⚪';
    });
    const overall = results.every(r => r === '✅') ? '✅ PASS' : '❌ FAIL';
    logBoth(`${calName.padEnd(8)} | ${results[0].padEnd(6)} | ${results[1].padEnd(6)} | ${results[2].padEnd(6)} | ${results[3].padEnd(6)} | ${results[4].padEnd(6)} | ${overall}`);
  });
  ...
  logBoth(`   ./get_all_caldav_events.sh | grep -i "verify.*${timestamp.toString().substring(8)}" | wc -l`);
  ...
  logBoth('     -H "Authorization: Basic $(echo -n \'jswans33@gmail.com:qrdq-tahw-xski-ogbf\' | base64)" \\\\');
  logBoth('     https://p36-caldav.icloud.com/1110188709/calendars/home/ | grep SUMMARY');
  
  // DETAILED TEST LOG
  logBoth('\\n📋 DETAILED TEST LOG');
  logBoth('====================');
  testResults.forEach((result, index) => {
    logBoth(`${(index + 1).toString().padStart(2, '0')}. ${result.timestamp} | ${result.result} | ${result.operation} ${result.calendar} | ${result.details}`);
  ...
  logBoth('\\n🎯 FINAL RESULT:');
  logBoth(allPassed ? '✅ ALL CRUD OPERATIONS VERIFIED SUCCESSFUL' : '❌ SOME OPERATIONS FAILED');
  logBoth(`📊 Success Rate: ${testResults.filter(r => r.result === '✅ PASS').length}/${testResults.length} operations`);
