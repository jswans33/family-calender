...
async function testMultiCalendarRepository() {
  ...
  try {
    ...
    calendars.forEach(cal => {
      ...
      console.log(
        `  ${status} ${cal.displayName} (${cal.name}): ${cal.count} events`
      );
    });

    const totalExpected = 320; // From our previous analysis
    const totalFound = calendars
      .filter(c => c.count >= 0)
    ...
    const eventsWithCalendarData = allEvents.filter(
      e => e.calendar_path && e.calendar_name && e.caldav_filename
    );
    console.log(
      `  ✅ ${eventsWithCalendarData.length}/${allEvents.length} events have complete metadata`
    );

    const uniqueIds = new Set(allEvents.map(e => e.id));
    console.log(`  ✅ ${uniqueIds.size}/${allEvents.length} unique event IDs`);

    const icsFilenames = allEvents.filter(
      e => e.caldav_filename && e.caldav_filename.endsWith('.ics')
    );
    console.log(
      `  ✅ ${icsFilenames.length}/${allEvents.length} events have .ics filenames`
    );
    ...
    if (sampleEvent) {
      ...
      console.log(
        `  Calendar: ${sampleEvent.calendar_name} (${sampleEvent.calendar_path})`
      );
